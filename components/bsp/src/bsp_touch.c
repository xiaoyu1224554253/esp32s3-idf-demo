...
static void bsp_touch_map_to_landscape(uint16_t raw_x, uint16_t raw_y, uint16_t *lcd_x, uint16_t *lcd_y)
{
    if (raw_x >= TOUCH_PANEL_WIDTH) raw_x = TOUCH_PANEL_WIDTH - 1;
    if (raw_y >= TOUCH_PANEL_HEIGHT) raw_y = TOUCH_PANEL_HEIGHT - 1;

    *lcd_x = (raw_y * BSP_LCD_HOR_RES) / TOUCH_PANEL_HEIGHT;
    *lcd_y = BSP_LCD_VER_RES - 1 - ((raw_x * BSP_LCD_VER_RES) / TOUCH_PANEL_WIDTH);
}
...
esp_err_t bsp_touch_init(void)
{
    ...
    ret = i2c_master_bus_add_device(i2c_bus, &dev_cfg, &s_touch_dev);
    if (ret != ESP_OK) {
...
bool bsp_touch_read(uint16_t *x, uint16_t *y, bool *pressed)
{
    ...
    esp_err_t ret = i2c_master_transmit_receive(s_touch_dev, &reg, 1, buf, 5, 50);
    if (ret != ESP_OK) {
        return false;
    }
